...
class IndexView(ListView):
    ...
    # 投稿日時の降順で並べ替える
    queryset = ProductPost.objects.order_by('-posted_at')
    # 1ページに表示するレコードの件数
    paginate_by = 9
...
class CreateProductView(CreateView):
    ...
    success_url = reverse_lazy('kanri:post_done')
...
class CategoryView(ListView):
    ...
    def get_queryset(self):
      ...
      categories = ProductPost.objects.filter(
...
class UserView(ListView):
    ...
    def get_queryset(self):
      ...
      # filter(フィールド名=id)で絞り込む
      user_list = ProductPost.objects.filter(
...
class MypageView(ListView):
    ...
    def get_queryset(self):
      ...
      queryset = ProductPost.objects.filter(
...
class ProductDeleteView(DeleteView):
    ...
    # 処理完了後にマイページにリダイレクト
    success_url = reverse_lazy('kanri:mypage')
